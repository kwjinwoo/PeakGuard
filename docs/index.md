---
id: documentation-index
title: Documentation Index
type: index
status: active
related:
  - README.md
  - status.md
---

# Documentation Index

This is the complete catalog of durable PeakGuard Wiki documents. `README.md` explains reading order; this page answers what documentation exists.

## Entry point

- [Wiki home](README.md)

## Current system

- [Current status](status.md)
- [System architecture](architecture.md)
- [Operations](operations.md)

## Concepts

- [Concept index](concepts/README.md)
- [Domain model](concepts/domain-model.md)
- [Configuration](concepts/configuration.md)
- [Data contracts](concepts/data-contracts.md)
- [Alert catalog](concepts/alerts/README.md)
- [Price and ATH signals](concepts/alerts/price-signals.md)
- [Fetch failure reporting](concepts/alerts/fetch-failures.md)
- [Testing strategy](concepts/testing-strategy.md)
- [Glossary](concepts/glossary.md)

## Direction and history

- [Roadmap index](roadmap/README.md)
- [Now](roadmap/now.md)
- [Next](roadmap/next.md)
- [Later](roadmap/later.md)
- [Proposal index](proposals/README.md)
- [PROP-0001: Distinguish Gist read failure modes](proposals/PROP-0001-distinguish-gist-read-failures.md)
- [Decision index](decisions/README.md)
- [ADR-0001: CSV in a GitHub Gist](decisions/0001-csv-gist-persistence.md)

## Operations and learned context

- [Runbook index](runbooks/README.md)
- [Provider fetch failures](runbooks/provider-fetch-failures.md)
- [Gist history read failures](runbooks/gist-history-read-failures.md)
- [Work notes index](work-notes/README.md)
- [Work notes — 2026-07](work-notes/2026-07.md)

## Wiki maintenance

- [Wiki meta](meta/README.md)
- [Template index](meta/templates/README.md)
- [Proposal template](meta/templates/proposal.md)
- [Work note template](meta/templates/work-note.md)
- [ADR template](meta/templates/adr.md)

`docs/AGENTS.md` contains agent instructions and is intentionally not treated as a Wiki knowledge node.
